...
vector<X> pointContainer;
vector<X> velocityContainer;
...
void passagePoints(double x, double y, double z, double velocity){
    static int i = 0;
    X newX;
    newX.i = i++;
    newX.x = x;
    ...
    newX.z = z;
    newX.v = velocity;
    pointContainer.push_back(newX);
    velocityContainer.push_back(newX);
}
void printPoints(){
    for(int i = 0; i<pointContainer.size(); i++){
        cout<<"point "<<pointContainer[i].i<<"\tx:"<<pointContainer[i].x<< " y:" << pointContainer[i].y<<" z:" << pointContainer[i].z <<" velocity: "<<pointContainer[i].v<< endl;
    }
}
...
//    cout<<q<<endl;
    ...
    passagePoints(0,1,0,20);
    passagePoints(0,1,2,30);
    passagePoints(0,3,0,15);
    passagePoints(4,1,0,10);
